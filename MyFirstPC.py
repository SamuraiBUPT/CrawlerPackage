...
Mypath = os.path.split(os.path.realpath(__file__))[0]  # 首先截取当前文件的文件地址 然后通过split把路径与文件名分离，这里得到的就是MyFirstPC.py这个名字
...
for i in range(0, n):
    k += 1
    if k == 10:
        book.save(savepath)  # savepath在前面定义过了 这里是个字符串 即文件名  这句话就是保存这个工作簿  保存名字为savepath的名字
        k = 0
    r = requests.get(url, params=date)  # 这句话跟第一次见面时不同  第一次使用request.get()之后 返回的是一个字典类  这个是接口的响应  这种响应是json格式的的数据
    result = r.json()
    ...
    if not os.path.exists(Mypath + '\\Mysetu\\U18'):
        os.mkdir(Mypath + '\\Mysetu\\U18')
    ...
    if result['data'][0]['r18'] == False:
        open(setupath + '\\U18' + '\\' + result['data'][0]['title'] + '.' + result['data'][0]['ext'], 'wb').write(myfile.content)
