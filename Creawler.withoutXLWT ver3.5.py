...
Mypath = os.path.split(os.path.realpath(__file__))[0]  # 首先截取当前文件的文件地址 然后通过split把路径与文件名分离，这里得到的就是MyFirstPC.py这个路径
...
for i in range(0, n):
    k += 1
    if k == 10:
        #book.save(savepath)  # savepath在前面定义过了 这里是个字符串 即文件名  这句话就是保存这个工作簿  按照这个路径进行保存
        k = 0
    ...
    try:
        if result['data'][0]['r18'] == True:
            open(setupath + '\\R18' + '\\' + result['data'][0]['title'] + '.' + result['data'][0]['ext'], 'wb').write(
                myfile.content)  # 打开这个路径 形式为'wb',把爬取到的东西写进去  就是myfile.content
    except:
        pass
